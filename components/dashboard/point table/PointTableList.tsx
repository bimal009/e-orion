...
import { PointsTable } from "@/lib/types";
...
const PointTableList = ({ tournmentId }: { tournmentId: string }) => {
  ...
  const [editPointTable, setEditPointTable] = useState<PointsTable | null>(
    null
  );
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <div className="flex items-center gap-2 ml-4">
            <Button
              ...
              variant="outline"
              className="text-xl font-semibold bg-transparent hover:bg-transparent hover:text-primary cursor-pointer text-primary shadow-none rounded-none"
            >
              Rounds
            </Button>
            <Button
      ...
      {!pointTables || pointTables.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mb-6">
            Get started by creating your first Point Tables
          </p>
          <RoundButton
            type={formType}
            onClick={handleCreate}
