...
import { useDeleteGroup } from '../api/useGroup'
...
const GroupCard = ({ group, onEdit ,roundId, tournmentId}: GroupCardProps) => {
    ...
    return (
      <div
        ...
        onClick={() => router.push(`/dashboard/tournment/${tournmentId}/${roundId}/groups/${group.id}`)}
      >
        {/* Background Gradient */}
              ...
              {menuOpen && (
                <div ref={menuRef} className="absolute right-0 mt-2 w-40 bg-popover backdrop-blur-sm border border-border rounded-xl shadow-xl py-2 z-50">
                  <button
                    ...
                    onClick={e => { e.stopPropagation(); setMenuOpen(false); onEdit && onEdit(group); }}
                  >
                  ...
                  <button
                    ...
                    onClick={e => { e.stopPropagation(); setMenuOpen(false); group.id && handleDelete(group.id); }}
                    disabled={isPending}
                  >
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between p-3 bg-accent rounded-xl border border-border">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium text-sm text-foreground">Matches</span>
              </div>
              <span className="font-bold text-primary text-lg">
