...
const GroupList = ({tournmentId,roundId}:{tournmentId:string,roundId:string}) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <Button onClick={() => router.push(`/dashboard/tournment/${tournmentId}/${roundId}`)} variant="outline" className="text-xl font-semibold bg-transparent hover:bg-transparent hover:text-primary cursor-pointer border-b-2 border-transparent">Games</Button>
          <Button onClick={() => router.push(`/dashboard/tournment/${tournmentId}/${roundId}/groups`)} variant="outline" className="text-xl font-semibold bg-transparent hover:bg-transparent hover:text-primary cursor-pointer border-b-2 border-primary text-primary">Groups</Button>
          </div>
        </div>
      ...
      { !groups || groups.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {groups?.map((group:Group) => (
            <GroupCard key={group.id?.toString()} group={group} onEdit={handleEdit} roundId={roundId} tournmentId={tournmentId} />
          ))}
        </div>
      )}
    </div>
