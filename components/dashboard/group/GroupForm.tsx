...
const GroupForm = ({
  ...
  const displayTeams =
    type === "edit"
      ? teamsData
      : teamsData?.filter((team: any) => team.isSelected === false);

  React.useEffect(() => {
    if (initialData) {
      reset({
        ...initialData,
        teams: initialData.teams?.map((t: any) => t.id) || [],
      });
    } else {
      reset({
        name: "",
  ...
  const { mutate, isPending } =
    type === "edit" ? useUpdateGroup() : useCreateGroup();
  ...
  const handleRandomTeamSelection = () => {
    if (displayTeams && randomTeamCount > 0) {
      ...
      const availableTeams =
        type === "edit"
          ? displayTeams.filter(
              (team: any) => !selectedTeams?.includes(team.id)
            )
          : displayTeams;
      ...
      const randomlySelected = shuffled
        .slice(0, maxCount)
      ...
      toast.success(
        `Randomly selected ${maxCount} team${maxCount > 1 ? "s" : ""}`
      );
    ...
    >
      <DialogContent className="sm:max-w-4xl max-w-[95vw] rounded-2xl max-h-[90vh] flex flex-col">
        <DialogHeader className="flex-shrink-0">
          ...
          <DialogDescription>
            {type === "edit"
              ? "Update the details below to edit the group."
            ...
            <div className="space-y-6 p-1">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    <Label htmlFor="randomCount" className="text-sm">
                      Number of teams:
                    </Label>
                    <Input
                      type="number"
                ...
                {randomTeamCount > 0 &&
                  displayTeams &&
                  randomTeamCount > displayTeams.length && (
                    <div className="flex items-center gap-2 p-2 bg-yellow-500/10 border border-yellow-500/20 rounded-md">
                      <div className="w-2 h-2 bg-yellow-500 rounded-full"></div>
                      <p className="text-xs text-yellow-600 dark:text-yellow-400">
                        Warning: You requested {randomTeamCount} teams but only{" "}
                        {displayTeams.length} are available. Only{" "}
                        {displayTeams.length} teams will be selected.
                      </p>
                    </div>
                  )}
              </div>
