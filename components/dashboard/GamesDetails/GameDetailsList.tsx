...
const GameDetailsPage = ({tournmentId,roundId,gameId}:{tournmentId:string,roundId:string,gameId:string}) => {
  const router = useRouter()
  const { data: game, isPending: isGamePending } = useGetGamesByGameId(gameId);
  console.log(game?.groupId, "game")
  const { data: groups, isPending: isGroupsPending } = useGetGroupsByGameId(gameId);
  console.log(groups?.flatMap(group => group.teams || []), "groups")
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <Button 
            onClick={() => router.push(`/dashboard/tournment/${tournmentId}/${roundId}`)} 
            variant="outline" 
            size="sm"
          ...
          <div className="flex items-center gap-2 ml-4">
            <Button onClick={() => router.push(`/dashboard/tournment/${tournmentId}/${roundId}/game/${gameId}`)} variant="outline" className="text-xl font-semibold bg-transparent hover:bg-transparent hover:text-primary cursor-pointer border-b-2 border-primary text-primary">Teams</Button>
            <Button onClick={() => router.push(`/dashboard/tournment/${tournmentId}/${roundId}/game/${gameId}/result`)} variant="outline" className="text-xl font-semibold bg-transparent hover:bg-transparent hover:text-primary cursor-pointer border-b-2 border-transparent">Result</Button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <GameDetailsCard 
          teams={groups?.flatMap(group => group.teams || []) || []} 
        />
      </div>
    </div>
  )
